...
void Graphics::DrawArrow(const Vector2f& from, const Vector2f& to, const Color& color)
{
	glBegin(GL_LINES);
	gl_Color(color);
	glVertex2f(from.x, from.y);
	...
	Vector2f headPoint = Vector2f::Normalize(to - from);
	headPoint.Rotate(45.0f * Math::DEG_TO_RAD);
	headPoint *= 30;
	...
	gl_Vertex(to);
	gl_Vertex(to - headPoint);

	headPoint.Rotate(-45.0f * 2 * Math::DEG_TO_RAD);
	gl_Vertex(to);
	gl_Vertex(to - headPoint);
...
void Graphics::DrawString(Font* font, const std::string& text, const Vector2f& pos, const Color& color, Align::value_type align)
{
	DrawString(font, text, pos.x, pos.y, color, align);
}
...
void Graphics::DrawString(Font* pFont, const std::string& text, float x, float y, const Color& color, Align::value_type align)
{
	...
	for (unsigned int i = 0; i < text.length(); i++)
	{
		...
		if (glyph.HasImage())
		{
			...
			float penX = Math::Floor(penPosition.x + 0.5f);
			float penY = Math::Floor(penPosition.y + 0.5f);
			DrawTextureRegion(pTexture,
...
Rect2f Graphics::MeasureString(Font* font,
	const std::string& text, Align::value_type align, float scale)
{
	Glyph glyph;
	Vector2f penPosition = Vector2f::ZERO;
	Vector2f mins = penPosition;
	Vector2f maxs = penPosition;
	...
	{
		glyph = font->GetGlyph(text[i]);
		mins.x = Math::Min(mins.x, penPosition.x + glyph.GetMinX());
		maxs.x = Math::Max(mins.x, penPosition.x + glyph.GetMaxX());
		mins.y = Math::Min(mins.y, penPosition.y + glyph.GetMinX());
		maxs.y = Math::Max(mins.y, penPosition.y + glyph.GetMaxX());
		penPosition.x += glyph.GetAdvance();
	}

	return Rect2f(mins, maxs - mins);
}
